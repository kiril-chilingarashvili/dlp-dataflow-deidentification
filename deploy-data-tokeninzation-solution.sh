...
gcloud dataflow jobs run ${jobId} --gcs-location gs://dataflow-templates/latest/Stream_DLP_GCS_Text_to_BigQuery --num-workers=1 --region=us-central1 \
...
export jobId="demo-dlp-deid-pipeline-`date +%Y%m%d-%H%M%S`"
gcloud dataflow jobs run ${jobId} --gcs-location gs://dataflow-templates/latest/Stream_DLP_GCS_Text_to_BigQuery --num-workers=1 --region=us-central1 \
    --parameters "inputFilePattern=gs://${DATA_STORAGE_BUCKET}/CCRecords_1564602826*.csv,dlpProjectId=${PROJECT_ID},deidentifyTemplateName=${DEID_TEMPLATE_NAME},inspectTemplateName=${INSPECT_TEMPLATE_NAME},datasetName=${BQ_DATASET_NAME},batchSize=500"
